...
- **定義文は挙動でなく目的で書く**：`{用語}とは、{どんな働きによって}{何を実現する}{名詞}である` の型で1文に言い切る。画面上のふるまいを「〜し、〜する」と並べない（動作が2つ以上並んだら冗長のサイン）。ユーザー視点で書き、システム側の仕様説明にしない。「〜してしまう」など評価も入れない。英語併記は本文初出で一度だけなら定義文の冒頭に置いてよい（禁止はタイトルへの併記だけ）。
  - 誤: `モーダル（Modal）とは、今の画面の上に重なって現れ、それに対応し終えるまで背後の操作をすべて止めてしまう画面要素である。`
  - 正: `モーダルとは、ユーザーの注意を強制的にひとつのことへ向けさせることで、操作を集中させるためのインターフェースである。`
  - 適用範囲は `excerpt`（最小限の説明）・導入リード文の1段落目・定義セクションの冒頭文。詳細は prompts/02_polishing.md 執筆ルール7。
- **1段落に論点をひとつだけ置く**：定義・具体例・語源・対概念を1段落に詰め込まない。1段落2〜4文が目安。各セクションの冒頭は結論を1文だけの独立段落にし、詳細は次の段落から書く。具体例は定義の直後に置く。1文に述語を3つ以上並べず、枝葉の固有名詞は落とす（詳細は prompts/02_polishing.md 執筆ルール8）。
...
- **語源・提唱者は年表にしない**：「①提唱当時、何が問題だったのか（誰が・どんな現場で・何に行き詰まっていたのか）→ ②その課題を解くために何を狙って作られたのか（目的をひとつ言い切る）→ ③結果、何が変わったのか」の順で書く。提唱当時の具体的エピソード（実際のプロジェクト・製品・失敗事例）は、それが無いと②の狙いが腑に落ちないときだけ入れる。提唱者の所属・肩書き・提唱年や名称の由来の豆知識は、知っても用語の使い方が変わらないなら書かない。「複雑さが増していた」のような一般論で①を済ませない（詳細は prompts/02_polishing.md の語源・提唱者セクション・CLAUDE.md 記事の書き方ルール9）。
- **効果・メリットは前後対比で書く**：「使わない場合に実務で何が起きるか」→「使うとどう変わるか」の順で対比を作る。「一貫性」「効率化」などの抽象語1語で済ませず、実務で何を意味するのかを後続の文で具体化する。誰に効くのか（エンドユーザー／作り手のチーム／事業）を書き分ける（詳細は CLAUDE.md 記事の書き方ルール10）。
- **記事後半に「実務でどう使われるか」を置く**：活用法はひとつに絞り、そのかわり「どの工程で・誰と・何がアウトプットとして残るか・ありがちなつまずきと回避策」の4点をすべて書く。前セクションの効能を言い換えるだけの記述にしない（詳細は CLAUDE.md 記事の書き方ルール11）。
