...
- **語源・提唱者は年表にしない**：「①提唱当時、何が問題だったのか（誰が・どんな現場で・何に行き詰まっていたのか）→ ②その課題を解くために何を狙って作られたのか（目的をひとつ言い切る）→ ③結果、何が変わったのか」の順で書く。提唱当時の具体的エピソード（実際のプロジェクト・製品・失敗事例）があれば必ず入れる。「複雑さが増していた」のような一般論で①を済ませない（詳細は prompts/02_polishing.md の語源・提唱者セクション・CLAUDE.md 記事の書き方ルール9）。
