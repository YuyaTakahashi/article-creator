...
### 用語の指定のしかた（表記ゆれはある程度そのまま通る）

`◯◯` のところは G-ID（`G-023`）でも用語名でもよい。用語名は用語DBの表記と一字一句同じでなくても、次の違いは用語くんが吸収して同じ行に当てる。

- 英語名（C列）でも引ける：`@用語くん Modal をWP下書きに`
- 全角・半角、大文字・小文字、括弧や中黒の有無：`ＩａＣ` `iac` `情報アーキテクチャ` `IA`
- 長音の有無、ひらがな・カタカナ：`ユーザービリティ` `ゆーざびりてぃ`
- `l` と `I`、`0` と `O` のような見分けにくい字の取り違え：`laC` → `IaC`

これでも決められないとき（近い用語が複数ある／1文字違い）は、取り違えないように「もしかしてこれ？」と候補のG-IDを出して聞き返す。出てきたG-IDで言い直せば進む。

### WP下書きの更新先が見つからないとき

一度WP下書きにした用語は、用語DBのQ列（`wp_post_id`）に投稿IDが記録され、次からは同じ投稿を更新する。その投稿がWPから消えていると、用語くんは新しい下書きを作り直してQ列のIDを差し替える（そのことをSlackで伝える）。設定と違う投稿タイプ（`posts` など）に同じIDの投稿が生きている場合は、記事が二重にならないよう作り直さずに止めるので、`WP_POST_TYPE` の設定かQ列のIDを確認する。ゴミ箱に入っていた場合は下書きに戻して更新する。
...
ジョブが緑なのに Slack の用語くんの挙動が変わらないときは、`@用語くん version` を叩く。版の文字列に続けて、デプロイ時に埋め込んだコミットSHA（`ビルド 74978dd` のような行）が返るので、`main` の最新コミットと突き合わせれば、いま動いているコードがどれか分かる。SHAが古ければデプロイの更新に失敗している（コードは push されていてもウェブアプリが古いバージョンを向いたまま）。版の文字列は手で書いているので、上げ忘れても中身は最新のことがある。判断はSHAで行う。
