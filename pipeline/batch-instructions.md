...
1. 貼り付け用のMDを組み立てる。手で作らず必ずスクリプトに作らせる。フロントマターの除去・`# {タイトル}`・レビュー案内の1行・**レシピ版の1行**をまとめて付ける

   ```bash
   python3 scripts/make_doc_md.py "drafts/{用語}.md"
   # 作り直し（U列=TRUE）のとき: --old-doc-url "{旧DocURL}"
   ```

   版の行はこの形で入る。どのDocを開いても、どのレシピで書かれた記事かがDocだけで分かるようにするためで、消さない：
